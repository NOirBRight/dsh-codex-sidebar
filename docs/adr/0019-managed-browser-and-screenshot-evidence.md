...
The production stream keeps the screencast as a change/ack source, but visible frames use a bounded `Page.captureScreenshot` clip at up to 1.5× scale, with latest-frame backpressure and a 20 FPS delivery cap. The binary header continues to report CSS viewport dimensions so input and 批注 coordinates remain CSS-pixel based.
