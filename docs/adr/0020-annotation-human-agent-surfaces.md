...
On DSH `0.1.2-alpha.4`, stacked chips occupy the official `conversation.input.dock` slot and use only its public `InputState` / `InputActions`. A chip is not published to that composer until its Host evidence batch has finished staging. When no visible draft or image exists, the dock writes one zero-width sentinel through `InputActions.setDraft`; the official submit path accepts it, and the Host removes it before the enriched message reaches transcript or model views. The Alpha.4 Session lifecycle snapshot contains no message history, so sent chips are retired from the binding's exact appended direct-user `user/message` event, not by inspecting private composer DOM or guessing from lifecycle bits.
