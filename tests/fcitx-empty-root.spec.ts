...
import { FCITX_EMPTY_ROOT_SEED, X11_LAUNCHER_FLAG, isX11Launcher, stripFcitxSeed } from '../src/client/fcitx-empty-root.ts'
...
describe('fcitx empty-root bridge', () => {
  it('activates only for the explicit X11 launcher query', () => {
    ...
    expect(isX11Launcher({ search: '' } as Location, { getItem: key => key === X11_LAUNCHER_FLAG ? '1' : null })).toBe(true)
  })
