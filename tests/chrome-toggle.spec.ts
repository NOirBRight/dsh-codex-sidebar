...
describe('侧栏开关 placement', () => {
  ...
  it('pins the handle to the details-column seam, not a CSS-var guess', () => {
    ...
    expect(css).toContain('[data-dcs-pin]')
    expect(css).not.toContain('[class$="_frame"]')
    expect(css).not.toContain('[class*="centerCol"]')
    expect(css).not.toContain('[class*="detailsCol"]')
    expect(css).not.toContain('[class$="_frame"]:has([data-shell-overlay])')
    const hostFrame = readFileSync(new URL('../src/client/host-frame.ts', import.meta.url), 'utf8')
    expect(hostFrame).toContain("setAttribute('data-dcs-details'")
    expect(hostFrame).toContain("setAttribute('data-dcs-header'")
    expect(hostFrame).not.toContain('class*=')
    expect(drawer).toContain('markHostFrame')
    expect(drawer).toContain('detailsColumnOf')
    expect(css).toContain('[data-dcs-header]')
    expect(css).toContain('[data-dcs-details]')
  })
})
