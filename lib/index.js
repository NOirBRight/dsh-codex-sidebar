...
function decodeBrowserClientMessage(raw) {
	...
	if (value.type === "media-decline") {
		if (identity === void 0 || value.reason !== "presentation-failed") return void 0;
		return {
			type: "media-decline",
			...identity,
			reason: value.reason
...
function sdpWithLoopbackHostCandidates(sdp) {
...
function candidateWithLoopbackHost(candidate) {
...
const MANAGED_BROWSER_DIRECT_VIDEO_MAX_BITRATE = 2e6;
...
var ManagedBrowserWebRtcEncoder = class {
	...
	constructor(opts) {
		...
		this.#frameRate = boundedInteger(opts.frameRate ?? 10, 1, 60, "frameRate");
		this.#maxBitrate = boundedInteger(opts.maxBitrate ?? 2e6, 1, 1e8, "maxBitrate");
	...
	async #start() {
		...
		try {
			...
			return {
				type: "offer",
				sdp: sdpWithLoopbackHostCandidates(offer.sdp)
			};
		} catch (error) {
...
const MANAGED_BROWSER_DIRECT_CAPTURE_MAX_RAW_BYTES = 491520;
...
function browserDirectCaptureProfile(config = {}) {
	return {
		quality: jpegQuality(config.directVideoCaptureQuality, 80, "directVideoCaptureQuality"),
...
var ManagedBrowserStream = class {
	...
	constructor(opts) {
		...
		this.#directVideoFrameRate = boundedStreamInteger(opts.directVideoFrameRate, 10, 1, 60, "directVideoFrameRate");
