...
function syncManagedBrowser(state, projection) {
	...
	const changedUrl = publicUrl.length > 0 && managedBrowserHref(publicUrl) !== void 0 && publicUrl !== current.url;
	const history = changedUrl ? [...current.history.slice(0, current.index + 1), publicUrl] : current.history;
	...
	return flags({
		...
		page: ready ? {
			url: publicUrl,
			title: projection.title || publicUrl,
			elements: []
...
function reduceBrowser(state, intent, port) {
	...
	switch (intent.type) {
		...
		case "browser-open-external": {
			...
			if (external !== void 0) port?.openExternal(external);
			return {
				state: current,
				effects: []
			};
		...
		case "browser-click-content": {
			...
			const click = intent;
			if (typeof click.captureId !== "string" || click.captureId.length === 0 || typeof click.documentId !== "string" || click.documentId.length === 0 || !Number.isSafeInteger(click.layoutRevision) || click.layoutRevision <= 0 || !Number.isSafeInteger(click.mediaGeneration) || click.mediaGeneration <= 0) return {
				state: current,
				effects: []
			};
...
function managedBrowserHref(url) {
	...
	if (parsed.protocol !== "file:" || parsed.host.length > 0 || parsed.username.length > 0 || parsed.password.length > 0) return void 0;
	let path;
	try {
	...
	return path.startsWith("/") && /\.html?$/i.test(path) ? parsed.href : void 0;
}
/** Chromium's failed-navigation page. Never treat this as the address the human asked for. */
function isChromiumErrorUrl(url) {
...
function loadPage(url, port) {
	...
	if (port === void 0 || managedBrowserHref(trimmed) !== void 0) return {
		url: trimmed,
		draft: trimmed,
		status: "loaded",
		page: {
...
function createHostBrowser(opts) {
	return {
		load(url) {
			...
			if (managedBrowserHref(url) === void 0) return void 0;
...
//#endregion
//#region lib/types/local-html-gateway.js
...
var LocalHtmlGateway = class {
	...
	async open(owner, rawUrl) {
		...
		if (lease === void 0 || lease.root !== entry.root) {
			if (lease !== void 0) this.#byToken.delete(lease.token);
		...
		} else lease.publicRoot = entry.publicRoot;
	...
	project(owner, navigationUrl) {
		...
		if (url.origin !== this.#origin || !url.pathname.startsWith(lease.internalPrefix)) return void 0;
		...
		if (!within(lease.publicRoot, publicPath)) return void 0;
	...
	isPrivate(navigationUrl) {
		if (this.#origin === void 0) return false;
		try {
			return new URL(navigationUrl).origin === this.#origin;
		} catch {
			return false;
		}
	}
	/** Remove private listener and capability identities from an error string. */
	redact(owner, message) {
		const lease = this.#byOwner.get(owner);
		let redacted = this.#origin === void 0 ? message : message.split(this.#origin).join("local-html://gateway");
		if (lease !== void 0) redacted = redacted.split(lease.token).join("<capability>");
	...
	resources() {
		return {
			listening: this.#server !== void 0,
			leases: this.#byOwner.size
		};
	}
	...
	async dispose() {
		...
		this.#byOwner.clear();
		this.#byToken.clear();
		if (this.#starting !== void 0) await this.#starting.catch(() => void 0);
	...
	async #listen() {
		if (this.#disposed) throw new Error("Local HTML gateway is disposed");
		if (this.#origin !== void 0) return this.#origin;
		if (this.#starting !== void 0) return this.#starting;
		const server = createServer((req, res) => {
			this.#respond(req, res).catch(() => {
				reply$1(res, 404);
			});
		});
		this.#server = server;
		this.#starting = new Promise((resolveStart, rejectStart) => {
			...
			const onListening = () => {
				...
				if (address === null || address.address !== "127.0.0.1") {
					server.close();
					rejectStart(/* @__PURE__ */ new Error("Local HTML gateway did not bind loopback"));
					return;
				}
	...
	async #respond(req, res) {
		...
		try {
			...
			if (!stat.isFile() || verifiedStat.isSymbolicLink() || !verifiedStat.isFile() || stat.dev !== verifiedStat.dev || stat.ino !== verifiedStat.ino) {
				await handle.close();
				reply$1(res, 404);
				return;
			}
...
async function localHtmlEntry(rawUrl) {
	if (!/^file:\/\/\/(?!\/)/i.test(rawUrl.trim())) throw new Error(LOCAL_HTML_ERROR);
	let url;
	try {
		url = new URL(rawUrl);
	} catch {
		throw new Error(LOCAL_HTML_ERROR);
	}
	...
	return {
		...
		publicRoot: dirname(path),
		name: basename(canonical)
	};
}
function decodeRelativePath(encoded) {
	let decoded;
	try {
...
function safeRelativePath(decoded) {
	if (decoded.length === 0 || decoded.includes("\0") || decoded.includes("\\")) return void 0;
	const parts = decoded.split("/");
	return parts.some((part) => part.length === 0 || part === "." || part === "..") ? void 0 : parts;
}
function within(root, path) {
	return path !== root && path.startsWith(root + sep);
}
function reply$1(res, status) {
	if (res.headersSent) {
		res.destroy();
		return;
	}
...
const MANAGED_BROWSER_WEBRTC_CHROMIUM_ARGS = ["--disable-features=WebRtcHideLocalIpsWithMdns"];
...
const DEFAULT_LAYOUT_PAINT_TIMEOUT_MS = 1e3;
const DEFAULT_BROWSER_CLEANUP_TIMEOUT_MS = 2e3;
const DEFAULT_LAYOUT_POLICY = Object.freeze({
	minViewport: Object.freeze({
		width: 320,
		height: 240
	}),
	maxViewport: Object.freeze({
		width: 1920,
		height: 1440
	}),
	settleMs: 180,
...
async function installViewportPaintBinding(cdp, binding) {
	const source = `(function () { const root = this; const NativePromise = root.Promise; const raf = root.requestAnimationFrame.bind(root); Object.defineProperty(root, ${JSON.stringify(binding)}, { configurable: false, enumerable: false, writable: false, value: () => new NativePromise((resolve) => { raf(() => { raf(resolve) }) }) }) })()`;
	await cdp.send("Page.enable");
	await cdp.send("Page.addScriptToEvaluateOnNewDocument", { source });
	assertRuntimeEvaluation(await cdp.send("Runtime.evaluate", {
		expression: source,
		returnByValue: true
	}), "Cannot install Browser viewport paint function");
}
var ManagedBrowserRuntime = class {
	...
	#pendingPages = /* @__PURE__ */ new Map();
	#ensureCommands = /* @__PURE__ */ new Map();
	#tabIdentities = /* @__PURE__ */ new Map();
	#requestedViewports = /* @__PURE__ */ new Map();
	#leases = /* @__PURE__ */ new Map();
	#captureSeq = 0;
	#onProjection;
	#onPopup;
	#targetInvalidationListeners = /* @__PURE__ */ new Set();
	#now;
	#maxLivePages;
	#idleMs;
	#cacheBudgetBytes;
	...
	#reaping = false;
	#layoutPolicy;
	#mediaPages = /* @__PURE__ */ new Set();
	#mediaPageReservations = 0;
	#maxEncoderPages;
	#layoutPaintTimeoutMs;
	#cleanupTimeoutMs;
	#localHtml;
	#disposed = false;
	#disposePromise;
	constructor(opts = {}) {
		...
		this.#onWarning = opts.onWarning ?? ((message) => {
			console.warn("[dsh-codex-sidebar] " + message);
		});
	...
	*/
	onTargetInvalidated(listener) {
		this.#targetInvalidationListeners.add(listener);
		return () => {
			this.#targetInvalidationListeners.delete(listener);
		};
	}
	list() {
	...
	layout(tab) {
		...
		return layout === void 0 ? void 0 : cloneLayout(layout);
	}
	...
	async #ensure(tab, url, key, identity) {
		this.#assertEnsureCurrent(key, identity);
		const publicUrl = managedBrowserHref(url);
		if (publicUrl === void 0) return failedProjection(tab, key, url, /^file:/i.test(url.trim()) ? "Only an absolute local HTML file can be opened" : "需要 http、https 或绝对本地 HTML 地址");
		let navigationUrl = publicUrl;
		if (publicUrl.startsWith("file:")) try {
			...
			return failedProjection(tab, key, publicUrl, errorMessage$1(error));
		}
		...
		await this.#enqueue(record, async () => {
			...
			this.#publish(record);
			try {
				await record.page.goto(navigationUrl, {
	...
	async closeSession(sessionId) {
		...
		for (const record of this.#pages.values()) if (record.tab.sessionId === sessionId) tabs.set(record.key, record.tab);
		for (const identity of this.#tabIdentities.values()) if (identity.tab.sessionId === sessionId) tabs.set(this.keyOf(identity.tab), identity.tab);
		for (const pending of this.#pendingPages.values()) if (pending.tab.sessionId === sessionId) tabs.set(this.keyOf(pending.tab), pending.tab);
	...
	async reap() {
		...
		try {
			const now = this.#now();
			for (const record of [...this.#pages.values()]) if (!this.#leased(record.key) && now - record.lastAccess >= this.#idleMs) await this.#closeTab(record.tab, false);
			...
			while (this.#pages.size > this.#maxLivePages && live.length > 0) {
				...
				if (oldest !== void 0) await this.#closeTab(oldest.tab, false);
			}
		} finally {
			this.#reaping = false;
		}
	...
	acquire(tab) {
		const key = this.keyOf(tab);
		const lease = {};
		const leases = this.#leases.get(key) ?? /* @__PURE__ */ new Set();
		leases.add(lease);
	...
	async reload(tab) {
		const record = this.#pages.get(this.keyOf(tab));
		if (record !== void 0 && isChromiumErrorUrl(record.page.url())) {
			const target = managedBrowserHref(record.url);
			if (target !== void 0) return this.ensure(tab, target);
		}
	...
	async resize(tab, width, height) {
		if (this.#pages.get(this.keyOf(tab)) === void 0) {
			this.#requestedViewports.set(this.keyOf(tab), normalizeFitViewport({
				width,
				height
			}, this.#layoutPolicy));
			return;
		}
		await this.proposeLayout(tab, {
			mode: "fit",
			viewport: {
				width,
				height
	...
	async proposeLayout(tab, proposal, expectedTarget) {
		const record = this.#pages.get(this.keyOf(tab));
		if (record === void 0) throw new Error(expectedTarget === void 0 ? "Browser page is not ready" : "Browser target is no longer current");
		if (expectedTarget !== void 0 && record.identity !== expectedTarget) throw new Error("Browser target is no longer current");
		...
		return await new Promise((resolve, reject) => {
			...
			} else record.pendingLayouts.push({
				kind: "proposal",
				proposal: normalized,
				waiters: [waiter]
			});
			this.#drainLayouts(record);
		});
	}
	/** Reapply one exact committed layout without creating a new revision. */
	async verifyLayout(tab, expected, expectedTarget) {
		const record = this.#pages.get(this.keyOf(tab));
		if (record === void 0 || record.identity !== expectedTarget) throw new Error("Browser target is no longer current");
		return await new Promise((resolve, reject) => {
			record.pendingLayouts.push({
				kind: "verification",
				expected: cloneLayout(expected),
				waiters: [{
					resolve,
					reject
				}]
	...
	async snapshot(tab) {
		...
		if (record === void 0) return notReady();
		const layoutEpoch = record.layoutEpoch;
		const documentId = record.documentId;
		this.#touch(record);
		const nodes = await this.#nodes(record, documentId, layoutEpoch);
		if (nodes === void 0 || !this.#recordReadable(record, void 0, layoutEpoch, documentId)) return notReady();
		return {
			url: record.url,
			title: record.title,
			driveable: true,
	...
	*/
	async outline(tab, expectedTarget) {
	...
	*/
	async trackRect(tab, selector, expectedTarget) {
		const record = this.#readableRecord(tab, expectedTarget);
		if (record === void 0) return notReady();
		const layoutEpoch = record.layoutEpoch;
		const documentId = record.documentId;
	...
	*/
	captureIdentity(tab, expectedTarget) {
		const record = this.#readableRecord(tab, expectedTarget);
		if (record === void 0) return void 0;
		return {
			documentId: record.documentId,
			layoutRevision: record.layout.revision,
			mediaGeneration: record.layout.mediaGeneration,
			layoutEpoch: record.layoutEpoch
		};
	}
	/** Resolve one exact target only when its committed viewport is safe for visual reads. */
	target(tab, expectedTarget) {
		const record = this.#pages.get(this.keyOf(tab));
		if (record === void 0 || record.page.isClosed()) return void 0;
		if (expectedTarget !== void 0 && record.identity !== expectedTarget) return void 0;
		if (expectedTarget === void 0 && (record.status === "error" || record.status === "crashed")) return void 0;
		if (record.layoutRunning) return void 0;
		return {
	...
	ownedTarget(tab, expectedTarget) {
		const record = this.#pages.get(this.keyOf(tab));
		if (record === void 0 || record.page.isClosed() || record.identity !== expectedTarget) return void 0;
		return {
			identity: record.identity,
			page: record.page,
			cdp: record.cdp,
	...
	*/
	async runInput(tab, expectedTarget, expectedLayout, action) {
		const record = this.#pages.get(this.keyOf(tab));
		if (record === void 0 || record.identity !== expectedTarget || record.page.isClosed()) return false;
		const release = await this.#acquireVisualOperation(record);
		try {
			if (!this.#recordReadable(record, expectedTarget, expectedLayout.layoutEpoch, expectedLayout.documentId) || record.layout.revision !== expectedLayout.revision) return false;
			await action(record.cdp, () => this.#recordCurrent(record, expectedTarget, expectedLayout.layoutEpoch, expectedLayout.documentId));
			return this.#recordCurrent(record, expectedTarget, expectedLayout.layoutEpoch, expectedLayout.documentId) && record.layout.revision === expectedLayout.revision;
		} finally {
			release();
		}
	}
	/** Lease one narrow media Page from the same persistent Chromium context. */
	async createMediaPage() {
		...
		const adapter = Object.freeze({
			exposeBinding: async (name, callback) => {
				...
				await page.exposeBinding(name, (_source, payload) => {
					callback({ page: adapter }, payload);
				});
			},
			evaluateFunction: async (source, argument) => {
				if (closed) throw new Error("Managed Browser media Page is closed");
				const json = JSON.stringify(argument);
				if (json === void 0) throw new Error("Managed Browser media Page accepts JSON arguments only");
				return page.evaluate("(" + source + ")(" + json + ")");
			},
			close: async () => {
				await lease.close();
			}
	...
	async #closeTab(tab, invalidateIdentity) {
		const key = this.keyOf(tab);
		...
		if (pending !== void 0) pending.cancelled = true;
		const record = this.#pages.get(key);
		if (record === void 0) return;
		this.#invalidateTarget(record);
		this.#pages.delete(key);
		rejectPendingLayout(record, /* @__PURE__ */ new Error("Browser page closed"));
	...
	async #dispose() {
		...
		this.#liveContext = void 0;
		this.#context = void 0;
		const localHtmlClose = this.#localHtml.dispose().catch((error) => {
			this.#onWarning("managed Browser local HTML shutdown failed: " + errorMessage$1(error));
		});
		const contextClose = context?.close().catch(() => void 0) ?? Promise.resolve();
		await this.#waitForCleanup(Promise.all([
			contextClose,
			localHtmlClose,
			...pages.map((record) => {
				rejectPendingLayout(record, /* @__PURE__ */ new Error("Managed Browser disposed"));
				return this.#cleanupRecord(record);
			}),
			...mediaPages.map((lease) => lease.close().catch(() => void 0))
		]).then(() => void 0));
	}
	async #record(tab) {
		const key = this.keyOf(tab);
	...
	async #createRecord(tab, pending) {
		const key = this.keyOf(tab);
		...
		try {
			...
			await this.#waitForCleanup(Promise.all([cdp?.detach().catch(() => void 0) ?? Promise.resolve(), page.isClosed() ? Promise.resolve() : page.close().catch(() => void 0)]).then(() => void 0));
			throw error;
		}
	}
	#commitRecord(tab, key, page, cdp, paintBinding, requestedViewport) {
		const record = {
	...
	async #ensureContext() {
		...
		const pending = (async () => {
			...
			const derivedCacheBytes = await estimateDerivedChromiumCacheBytes(this.profileDir).catch((error) => {
				this.#onWarning("managed Browser cache size estimate failed: " + errorMessage$1(error));
				return 0;
			});
			...
			context.on("close", () => {
				contextClosed = true;
				if (this.#liveContext === context) this.#liveContext = void 0;
				for (const pendingPage of this.#pendingPages.values()) if (pendingPage.context === context) pendingPage.cancelled = true;
				...
				for (const record of records) {
					...
					rejectPendingLayout(record, /* @__PURE__ */ new Error("Chromium context exited"));
					record.status = "crashed";
			...
			if (derivedCacheBytes > this.#cacheBudgetBytes) {
				try {
					...
					await assertContextAvailable();
					this.#onWarning("managed Browser cache clear failed: " + errorMessage$1(error));
				}
				await assertContextAvailable();
			}
			await assertContextAvailable();
	...
	#captureStillCurrent(record, documentId, layout, layoutEpoch) {
		return this.#recordReadable(record, void 0, layoutEpoch) && record.documentId === documentId && sameLayoutIdentity(record.layout, layout);
	}
	async #drainLayouts(record) {
		...
		try {
			...
			if (record.pendingLayouts.length > 0) this.#drainLayouts(record);
		}
	}
	async #applyViewport(record, viewport, verifyFirst = false) {
		try {
			...
			if (verifyFirst) {
				...
				if (!cssViewportMatches(await this.#cssViewport(record), viewport, deviceScaleFactor)) throw new Error("Chromium did not preserve the Browser viewport");
				await this.#waitForViewportPaint(record);
				return;
			}
			await record.page.setViewportSize(viewport);
			this.#assertLayoutRecordCurrent(record);
			if (cssViewportMatches(await this.#cssViewport(record), viewport, deviceScaleFactor)) {
				await this.#waitForViewportPaint(record);
				return;
			}
			await this.#applyDeviceMetrics(record, viewport, deviceScaleFactor);
			if (!cssViewportMatches(await this.#cssViewport(record), viewport, deviceScaleFactor)) throw new Error("Chromium did not apply the Browser viewport");
			await this.#waitForViewportPaint(record);
		} catch (error) {
	...
	async #applyDeviceMetrics(record, viewport, deviceScaleFactor) {
	...
	async #waitForViewportPaint(record) {
		...
		const paint = (async () => {
			...
			await capture();
			assertRuntimeEvaluation(await record.cdp.send("Runtime.evaluate", {
				expression: "this[" + JSON.stringify(record.paintBinding) + "]()",
				awaitPromise: true,
				returnByValue: true
			}), "Browser viewport paint function failed");
			await capture();
		})();
		let timer;
		const deadline = new Promise((_resolve, reject) => {
			timer = setTimeout(() => {
				reject(/* @__PURE__ */ new Error("Chromium Browser viewport paint timed out"));
			}, this.#layoutPaintTimeoutMs);
			timer.unref();
		});
		try {
	...
	async #cssViewport(record) {
		const metrics = await record.cdp.send("Page.getLayoutMetrics");
		this.#assertLayoutRecordCurrent(record);
		const viewport = metrics.cssLayoutViewport ?? metrics.layoutViewport;
		const width = viewport?.clientWidth;
		const height = viewport?.clientHeight;
		if (typeof width !== "number" || !Number.isFinite(width) || width <= 0 || typeof height !== "number" || !Number.isFinite(height) || height <= 0) throw new Error("Chromium did not report the Browser CSS viewport");
		return {
			width,
			height,
			deviceScaleFactor: record.deviceScaleFactor
	...
	#assertLayoutRecordCurrent(record) {
		if (this.#pages.get(record.key) !== record || record.page.isClosed()) throw new Error("Browser page closed during layout commit");
	}
	#readableRecord(tab, expectedTarget) {
		const record = this.#pages.get(this.keyOf(tab));
		return record !== void 0 && this.#recordReadable(record, expectedTarget) ? record : void 0;
	}
	#recordReadable(record, expectedTarget, expectedLayoutEpoch, expectedDocumentId) {
		return this.#recordCurrent(record, expectedTarget, expectedLayoutEpoch, expectedDocumentId) && !record.layoutRunning;
	}
	#recordCurrent(record, expectedTarget, expectedLayoutEpoch, expectedDocumentId) {
		return this.#pages.get(record.key) === record && record.status === "ready" && !record.page.isClosed() && (expectedTarget === void 0 || record.identity === expectedTarget) && (expectedLayoutEpoch === void 0 || record.layoutEpoch === expectedLayoutEpoch) && (expectedDocumentId === void 0 || record.documentId === expectedDocumentId);
	}
	async #acquireVisualOperation(record) {
		const previous = record.visualOperationTail;
		let release;
		record.visualOperationTail = new Promise((resolve) => {
			release = resolve;
		});
		await previous;
		return release;
	...
	#fail(record, error) {
		record.status = "error";
		record.error = this.#localHtml.redact(record.key, errorMessage$1(error));
		record.refs.clear();
		this.#publish(record);
	}
	#publish(record) {
	...
	#invalidateTarget(record) {
		for (const listener of this.#targetInvalidationListeners) try {
			listener(record.tab, record.identity);
		} catch (error) {
			this.#onWarning("managed Browser target invalidation observer failed: " + errorMessage$1(error));
		}
	}
	#assertPendingPageCurrent(pending, context) {
		if (pending.cancelled || this.#disposed || this.#liveContext !== context) throw new Error("Browser Page creation was cancelled");
	}
	...
	async #cleanupRecord(record) {
		await Promise.all([record.cdp.detach().catch(() => void 0), record.page.isClosed() ? Promise.resolve() : record.page.close().catch(() => void 0)]);
	}
	#publicPageUrl(record, navigationUrl) {
		return this.#localHtml.project(record.key, navigationUrl) ?? (this.#localHtml.isPrivate(navigationUrl) ? record.url : navigationUrl);
	}
	async #nodes(record, documentId, layoutEpoch) {
		const raw = await record.page.evaluate(SNAPSHOT_EXPRESSION);
		if (!this.#recordReadable(record, void 0, layoutEpoch, documentId)) return void 0;
		record.refs.clear();
	...
	async #act(tab, ref, action) {
		...
		const release = await this.#acquireVisualOperation(record);
		try {
			if (!this.#recordReadable(record, void 0, layoutEpoch, documentId)) return notReady();
	...
	#serializeEnsure(key, identity, command) {
		const result = (this.#ensureCommands.get(key) ?? Promise.resolve()).then(command, command);
		...
		settled.then(() => {
			...
			if (!this.#pages.has(key) && !this.#pendingPages.has(key) && this.#tabIdentities.get(key) === identity) this.#tabIdentities.delete(key);
		});
		return result;
	}
	#assertEnsureCurrent(key, identity) {
		if (this.#disposed) throw new Error("Managed Browser is disposed");
		if (this.#tabIdentities.get(key) !== identity) throw new Error("Browser Tab was closed while opening");
	}
};
function layoutPolicy(config) {
	...
	if (minViewport.width > maxViewport.width || minViewport.height > maxViewport.height) throw new Error("managedBrowser layoutMinViewport must not exceed layoutMaxViewport");
	return {
		minViewport,
		maxViewport,
...
function configuredSize(value, fallback, name) {
	if (value === void 0) return { ...fallback };
	if (!Number.isFinite(value.width) || !Number.isFinite(value.height) || value.width <= 0 || value.height <= 0 || value.width > 65535 || value.height > 65535) throw new Error("managedBrowser " + name + " must contain positive finite 16-bit dimensions");
	return {
		width: Math.round(value.width),
		height: Math.round(value.height)
	};
}
function configuredNonNegative(value, fallback, name) {
	if (value === void 0) return fallback;
	if (!Number.isFinite(value) || value < 0) throw new Error("managedBrowser " + name + " must be a non-negative finite number");
	return Math.round(value);
}
function configuredPositiveInteger(value, fallback, name) {
	if (value === void 0) return fallback;
	if (!Number.isSafeInteger(value) || value <= 0) throw new Error("managedBrowser " + name + " must be a positive safe integer");
	return value;
}
function normalizeLayoutProposal(proposal, policy) {
	const fixed = proposal.mode === "fit" ? void 0 : FIXED_VIEWPORTS[proposal.mode];
	return {
		mode: proposal.mode,
		viewport: fixed === void 0 ? normalizeFitViewport(proposal.viewport, policy) : { ...fixed }
	};
}
function normalizeFitViewport(viewport, policy) {
...
function cssViewportMatches(actual, expected, deviceScaleFactor) {
...
function rejectPendingLayout(record, error) {
...
async function estimateDerivedChromiumCacheBytes(profileDir) {
	let total = 0;
	for (const segments of CHROMIUM_DERIVED_CACHE_DIRS) {
		...
		if (info === void 0 || info.isSymbolicLink() || !info.isDirectory()) continue;
		total += await directoryBytesWithoutSymlinks(path);
	}
	return total;
}
async function clearChromiumBrowserCache(context) {
	const page = await context.newPage();
	let cdp;
	const failures = [];
	try {
		...
		if (cdp !== void 0) await cdp.detach().catch((error) => {
			failures.push(error);
		});
...
async function directoryBytesWithoutSymlinks(path) {
	let bytes = 0;
	...
	for (const entry of entries) {
		...
		if (info === void 0 || info.isSymbolicLink()) continue;
		if (info.isDirectory()) {
...
function assertRuntimeEvaluation(value, message) {
...
function cacheBudgetBytes(value) {
	const resolved = value ?? 268435456;
...
function failedProjection(tab, key, url, error) {
	return {
		key,
...
function staleLayout() {
	return {
		ok: false,
		code: "stale-layout",
		message: "Browser document or layout changed; capture again"
	};
}
...
const MANAGED_BROWSER_EVIDENCE_CHUNK_BYTES = 98304;
var ManagedBrowserEvidenceStore = class {
	...
	async commit(sessionId, captureId, expected) {
		this.#pruneTemporary();
		...
		if (committed !== void 0) {
			...
			if (committed.evidence.layoutRevision !== expected.revision || committed.evidence.mediaGeneration !== expected.mediaGeneration) throw new Error("Browser capture layout identity is stale");
		...
		if (capture.layoutRevision !== expected.revision || capture.mediaGeneration !== expected.mediaGeneration || !sameCaptureIdentity(this.#runtime.captureIdentity(temporary.tab, capture.targetIdentity), capture)) {
		...
		this.#captures.delete(captureId);
		if (!sameCaptureIdentity(this.#runtime.captureIdentity(temporary.tab, capture.targetIdentity), capture)) throw new Error("Browser capture is stale after navigation or layout change");
		const evidence = {
			id,
			captureId,
	...
	async readChunk(sessionId, evidence, offset) {
...
function sameCaptureIdentity(current, capture) {
	return current !== void 0 && current.documentId === capture.documentId && current.layoutRevision === capture.layoutRevision && current.mediaGeneration === capture.mediaGeneration && current.layoutEpoch === capture.layoutEpoch;
}
//#endregion
//#region lib/types/managed-browser-session-lifecycle.js
/**
...
*/
function installManagedBrowserSessionLifecycle(ctx, stream, runtime, filesBySession) {
	return ctx.on("session/disposed", (session) => {
		stream.closeSession(session.id);
		filesBySession.delete(session.id);
		runtime.closeSession(session.id).catch((error) => {
...
const MANAGED_BROWSER_MEDIA_HIDE_GRACE_MS = 15e3;
const MAX_BROWSER_CONTROL_MESSAGE_BYTES = 131072;
const MAX_RTC_SDP_LENGTH = 65536;
const MAX_RTC_CANDIDATE_LENGTH = 4096;
const MAX_RTC_CANDIDATE_FIELD_LENGTH = 256;
/** Decode one untrusted control message from the Browser WebSocket. */
function decodeBrowserClientMessage(raw) {
	if (!controlMessageWithinLimit(raw)) return void 0;
	let value;
	try {
	...
	if (!record(value) || typeof value.type !== "string") return void 0;
	if (value.type === "hello") {
		if (value.version !== 2 || !Array.isArray(value.frameEncodings) || !Array.isArray(value.flowControl) || !record(value.media) || typeof value.media.webrtcVideo !== "boolean") return void 0;
		const frameEncodings = value.frameEncodings.filter((item) => item === "binary-v2" || item === "json-base64-v2");
		if (frameEncodings.length !== value.frameEncodings.length || value.flowControl.length !== 1 || value.flowControl[0] !== "frame-ack-v2") return void 0;
		return {
			type: "hello",
			version: 2,
			frameEncodings,
			flowControl: ["frame-ack-v2"],
			media: { webrtcVideo: value.media.webrtcVideo }
		};
	}
	if (value.type === "layout-propose") {
		if (!positiveSafeInteger$1(value.proposalSequence) || !layoutMode(value.mode) || !size(value.viewport)) return void 0;
		return {
			type: "layout-propose",
	...
	if (value.type === "input") {
		if (!positiveSafeInteger$1(value.revision) || !browserInput(value.input)) return void 0;
		return {
			type: "input",
	...
	if (value.type === "frame-ack") {
		if (!positiveSafeInteger$1(value.sequence) || !positiveSafeInteger$1(value.revision) || !positiveSafeInteger$1(value.mediaGeneration)) return void 0;
		return {
			type: "frame-ack",
	...
	if (value.type === "rtc-answer") {
		if (identity === void 0 || !rtcDescription(value.description, "answer")) return void 0;
		return {
			type: "rtc-answer",
			...identity,
	...
	if (value.type === "rtc-candidate") {
		if (identity === void 0 || !rtcCandidateOrNull(value.candidate)) return void 0;
		return {
			type: "rtc-candidate",
			...identity,
	...
	if (value.type === "media-retry") {
		if (identity === void 0 || value.trigger !== "explicit" && value.trigger !== "network-change" && value.trigger !== "tab-reactivate") return void 0;
		return {
			type: "media-retry",
			...identity,
	...
	if (value.type === "media-decline") {
		if (identity === void 0 || value.reason !== "presentation-failed" && value.reason !== "negotiation-timeout" && value.reason !== "negotiation-error" && value.reason !== "ready-missing" && value.reason !== "ready-owner-mismatch" && value.reason !== "receiver-sync-failed" && value.reason !== "remote-description-failed" && value.reason !== "candidate-failed" && value.reason !== "answer-failed" && value.reason !== "local-description-failed" && value.reason !== "peer-failed") return void 0;
		if (value.detail !== void 0 && (typeof value.detail !== "string" || value.detail.length > 256)) return void 0;
		return {
			type: "media-decline",
			...identity,
			reason: value.reason,
	...
	if (value.type === "surface-visibility") {
		if (identity === void 0 || typeof value.visible !== "boolean") return void 0;
		return {
	...
	return value.type === "outline" ? { type: "outline" } : void 0;
}
/** Encode one binary JPEG frame with layout and media identity. */
function encodeBrowserStreamFrameV2(frame) {
	const header = /* @__PURE__ */ new Uint8Array(29);
...
function encodeBrowserStreamJsonFrameV2(frame) {
...
function browserInput(value) {
	if (!record(value) || typeof value.type !== "string") return false;
	if (value.type === "text") return typeof value.text === "string";
	if (value.type === "keyDown" || value.type === "keyUp") return typeof value.key === "string" && typeof value.code === "string" && (value.modifiers === void 0 || nonNegativeSafeInteger(value.modifiers));
	if (!finite(value.x) || !finite(value.y)) return false;
	if (value.type === "wheel") return finite(value.deltaX) && finite(value.deltaY) && (value.selector === void 0 || typeof value.selector === "string");
	if (value.type === "drag") return finite(value.toX) && finite(value.toY);
	if (value.type === "move") return value.pressed === void 0;
	return value.type === "tap";
}
function controlMessageWithinLimit(raw) {
	return raw.length <= MAX_BROWSER_CONTROL_MESSAGE_BYTES && new TextEncoder().encode(raw).byteLength <= MAX_BROWSER_CONTROL_MESSAGE_BYTES;
}
function mediaIdentity(value) {
	if (typeof value.ownerId !== "string" || value.ownerId.length === 0 || value.ownerId.length > 256 || !positiveSafeInteger$1(value.revision) || !positiveSafeInteger$1(value.mediaGeneration)) return void 0;
	return {
...
function rtcDescription(value, type) {
	return record(value) && value.type === type && typeof value.sdp === "string" && value.sdp.length > 0 && value.sdp.length <= MAX_RTC_SDP_LENGTH;
}
function rtcCandidateOrNull(value) {
	if (value === null) return true;
	return record(value) && typeof value.candidate === "string" && value.candidate.length <= MAX_RTC_CANDIDATE_LENGTH && (value.sdpMid === void 0 || value.sdpMid === null || typeof value.sdpMid === "string" && value.sdpMid.length <= MAX_RTC_CANDIDATE_FIELD_LENGTH) && (value.sdpMLineIndex === void 0 || value.sdpMLineIndex === null || nonNegativeSafeInteger(value.sdpMLineIndex)) && (value.usernameFragment === void 0 || value.usernameFragment === null || typeof value.usernameFragment === "string" && value.usernameFragment.length <= MAX_RTC_CANDIDATE_FIELD_LENGTH);
}
...
//#endregion
//#region lib/types/managed-browser-loopback-stun.js
/** Loopback-only STUN so GUI Chrome and the encoder Page can ICE on 127.0.0.1. */
const BINDING_REQUEST = 1;
const BINDING_SUCCESS = 257;
const MAGIC_COOKIE = 554869826;
const XOR_MAPPED_ADDRESS = 32;
const IPV4 = 1;
/** Bind a STUN responder on 127.0.0.1 so both WebRTC peers share a loopback srflx pair. */
function startLoopbackStunServer() {
	return new Promise((resolve, reject) => {
		...
		socket.on("message", (message, remote) => {
			...
			if (response !== void 0) socket.send(response, remote.port, remote.address);
		});
...
function stunBindingSuccess(request, ip, port) {
	if (request.byteLength < 20) return void 0;
	const view = Buffer.from(request.buffer, request.byteOffset, request.byteLength);
	if (view.readUInt16BE(0) !== BINDING_REQUEST || view.readUInt32BE(4) !== MAGIC_COOKIE) return void 0;
	...
	if (a === void 0 || b === void 0 || c === void 0 || d === void 0 || [
		...
		c,
		d
	].some((part) => !Number.isInteger(part) || part < 0 || part > 255)) return void 0;
	...
	response.writeUInt16BE(port ^ 8466, 26);
	response.writeUInt8((a ^ 33) & 255, 28);
	response.writeUInt8((b ^ 8466) & 255, 29);
	response.writeUInt8((c ^ 2167460) & 255, 30);
	response.writeUInt8((d ^ MAGIC_COOKIE) & 255, 31);
	return response;
}
//#endregion
//#region lib/types/managed-browser-ice-loopback.js
...
function extraHostIceAddresses() {
	const extras = ["127.0.0.1"];
	for (const addrs of Object.values(networkInterfaces())) for (const addr of addrs ?? []) {
...
function sdpWithLoopbackHostCandidates(sdp, extras = extraHostIceAddresses()) {
...
function candidateWithLoopbackHost(candidate, extras = extraHostIceAddresses()) {
	const match = HOST_CAND.exec(candidate);
	if (match === null) return [];
	const port = match[7];
	return extras.filter((ip) => ip !== match[6]).map((ip, index) => match[1] + match[2] + "x" + index + " " + match[3] + " " + match[4] + " " + match[5] + " " + ip + " " + port + " typ host" + (match[8] ?? ""));
}
function isClashFakeIp(ip) {
	const [a, b] = ip.split(".").map((part) => Number(part));
	return a === 198 && b !== void 0 && b >= 18 && b <= 19;
}
const MANAGED_BROWSER_DIRECT_VIDEO_MAX_BITRATE = 8e6;
/** STUN-only default so GUI Chrome and the encoder Page can form srflx pairs in addition to host ICE. */
const MANAGED_BROWSER_DEFAULT_STUN_URLS = ["stun:stun.l.google.com:19302"];
...
function validateBrowserStunUrls(urls) {
	return urls.map((value) => {
		...
		if (parsed.protocol !== "stun:" || parsed.pathname.length === 0 || parsed.search.length > 0 || parsed.hash.length > 0) throw new Error("Managed Browser WebRTC accepts valid STUN URLs only");
		return value;
	});
}
/** One immutable Browser owner/generation and its Chromium media Page. */
var ManagedBrowserWebRtcEncoder = class {
	identity;
	#pageFactory;
	#stunUrls;
	#width;
	...
	constructor(opts) {
		...
		this.#frameRate = boundedInteger(opts.frameRate ?? 20, 1, 60, "frameRate");
		this.#maxBitrate = boundedInteger(opts.maxBitrate ?? 8e6, 1, 1e8, "maxBitrate");
		this.#onSignal = opts.onSignal ?? (() => {});
	}
	/** Create the owned Page and return its SDP offer. */
	start() {
		if (this.#disposed) return Promise.reject(/* @__PURE__ */ new Error("Managed Browser WebRTC encoder is disposed"));
	...
	async #start() {
		...
		try {
			await page.exposeBinding(SIGNAL_BINDING, (source, payload) => {
				...
				if (signal === void 0 || signal.type === "candidate" && !this.#offerReady) return;
				this.#emit(signal);
			...
			const offer = await page.evaluateFunction(COMMAND, { type: "create-offer" });
			...
			if (this.#page === page) this.#page = void 0;
			await page.close().catch(() => void 0);
			throw error;
		}
	}
	async #command(command) {
		await (await this.#readyPage()).evaluateFunction(COMMAND, command);
	}
	async #readyPage() {
		if (this.#disposed) throw new Error("Managed Browser WebRTC encoder is disposed");
		await this.start();
		const page = this.#page;
		if (page === void 0 || this.#disposed) throw new Error("Managed Browser WebRTC encoder is disposed");
		return page;
	}
	#pump() {
		if (this.#disposed || !this.#connected || this.#paintPromise !== void 0 || this.#dirty === void 0 || this.#page === void 0) return;
		const frame = this.#dirty;
		const page = this.#page;
		this.#dirty = void 0;
		this.#paintPromise = page.evaluateFunction(COMMAND, {
			type: "paint",
			...frame
		}).then(() => {
			...
			if (!this.#disposed && this.#page === page) this.#emit({
				type: "encoder-error",
				message: errorMessage(error)
			});
		}).finally(() => {
			this.#paintPromise = void 0;
			this.#pump();
		});
	}
	#emit(signal) {
		...
		for (const loopback of candidateWithLoopbackHost(signal.candidate.candidate)) this.#onSignal({
			...this.identity,
			signal: {
				type: "candidate",
				candidate: {
	...
	#armIcePoll() {
		const tick = async () => {
			if (this.#disposed || this.#connected || this.#page === void 0) return;
			try {
				const ice = await this.#page.evaluateFunction(COMMAND, { type: "ice-state" });
	...
	async #dispose() {
		...
		if (this.#icePollTimer !== void 0) clearTimeout(this.#icePollTimer);
		this.#icePollTimer = void 0;
		this.#dirty = void 0;
		if (this.#startPromise !== void 0) await this.#startPromise.catch(() => void 0);
		const page = this.#page;
		this.#page = void 0;
		if (page === void 0) return;
		await page.evaluateFunction(COMMAND, { type: "dispose" }).catch(() => void 0);
		await page.close().catch(() => void 0);
	}
};
function browserMediaPageSignal(value) {
	if (typeof value !== "object" || value === null) return void 0;
	const signal = value;
	if (signal.type === "connection-state" && connectionState(signal.state)) return {
		type: "connection-state",
		state: signal.state
	};
	if (signal.type !== "candidate") return void 0;
	if (signal.candidate === null) return {
		type: "candidate",
		candidate: null
	};
	if (!browserRtcCandidate(signal.candidate)) return void 0;
	return {
		type: "candidate",
		candidate: signal.candidate
	};
}
function browserRtcDescription(value, expected) {
	if (typeof value !== "object" || value === null) return false;
	const description = value;
	return description.type === expected && typeof description.sdp === "string";
}
function browserRtcCandidate(value) {
	if (typeof value !== "object" || value === null) return false;
	const candidate = value;
	return typeof candidate.candidate === "string" && (candidate.sdpMid === void 0 || candidate.sdpMid === null || typeof candidate.sdpMid === "string") && (candidate.sdpMLineIndex === void 0 || candidate.sdpMLineIndex === null || typeof candidate.sdpMLineIndex === "number") && (candidate.usernameFragment === void 0 || candidate.usernameFragment === null || typeof candidate.usernameFragment === "string");
}
...
function sourcePage(source) {
	return typeof source === "object" && source !== null ? source.page : void 0;
}
...
function boundedInteger(value, min, max, name) {
	if (!Number.isSafeInteger(value) || value < min || value > max) throw new Error("Managed Browser media " + name + " must be an integer from " + min + " to " + max);
	return value;
}
function errorMessage(error) {
	return error instanceof Error ? error.message : String(error);
}
const MANAGED_BROWSER_STREAM_VERSION = 2;
const TICKET_TTL_MS = 3e4;
const MAX_BUFFERED_BYTES = 524288;
const MANAGED_BROWSER_STREAM_SHUTDOWN_TIMEOUT_MS = 2e3;
const HIGH_DENSITY_SCALE = 1.5;
const MANAGED_BROWSER_MOBILE_MAX_RAW_BYTES = 98304;
const MANAGED_BROWSER_DESKTOP_MAX_RAW_BYTES = 491520;
const MANAGED_BROWSER_DIRECT_CAPTURE_MAX_RAW_BYTES = 1048576;
const MANAGED_BROWSER_DIRECT_CAPTURE_MAX_SCALE = 1.5;
const MANAGED_BROWSER_MEDIA_IDLE_TIMEOUT_MS = 3e5;
/** Resolve the encoder capture independently from the socket's fallback transport. */
function browserDirectCaptureProfile(config = {}) {
	return {
		quality: jpegQuality(config.directVideoCaptureQuality, 90, "directVideoCaptureQuality"),
...
function browserStreamTransportProfile(route, config = {}) {
	return route === "mobile" ? {
		frameEncoding: "json-base64-v2",
		quality: jpegQuality(config.mobileJpegQuality, 65, "mobileJpegQuality"),
		maxScale: jpegScale(config.mobileJpegMaxScale, 1, "mobileJpegMaxScale"),
		frameIntervalMs: positiveStreamInteger(config.mobileJpegFrameIntervalMs, 250, "mobileJpegFrameIntervalMs"),
		everyNthFrame: positiveStreamInteger(config.mobileScreencastEveryNthFrame, 4, "mobileScreencastEveryNthFrame"),
		interactionBurstFrames: boundedStreamInteger(config.mobileJpegInteractionBurstFrames, 4, 0, 600, "mobileJpegInteractionBurstFrames"),
		maxRawBytes: rawByteBudget(config.mobileJpegMaxRawBytes, MANAGED_BROWSER_MOBILE_MAX_RAW_BYTES)
	} : {
		frameEncoding: "binary-v2",
		quality: jpegQuality(config.desktopJpegQuality, 80, "desktopJpegQuality"),
		maxScale: jpegScale(config.desktopJpegMaxScale, HIGH_DENSITY_SCALE, "desktopJpegMaxScale"),
		frameIntervalMs: positiveStreamInteger(config.desktopJpegFrameIntervalMs, 100, "desktopJpegFrameIntervalMs"),
		everyNthFrame: positiveStreamInteger(config.desktopScreencastEveryNthFrame, 2, "desktopScreencastEveryNthFrame"),
		interactionBurstFrames: boundedStreamInteger(config.desktopJpegInteractionBurstFrames, 20, 0, 600, "desktopJpegInteractionBurstFrames"),
...
function browserStreamCaptureDelay(lastCapturedAt, now, frameIntervalMs) {
	if (lastCapturedAt === void 0) return 0;
...
var BrowserFallbackActivityBudget = class {
	#limit;
	#remaining = 0;
	constructor(limit) {
...
var ManagedBrowserStream = class {
	...
	#tabConnections = /* @__PURE__ */ new Map();
	#socketCleanup = /* @__PURE__ */ new Map();
	#timerCount = 0;
	#captureCount = 0;
	#unackedCount = 0;
	#profiles;
	#directCaptureProfile;
	#tasks = /* @__PURE__ */ new Set();
	#preferredMediaRoute;
	#stunUrls;
	#loopbackStun;
	#loopbackStunTask;
	#webrtcNegotiationTimeoutMs;
	#webrtcRetryCooldownMs;
	#maxMediaPeers;
	#directVideoFrameRate;
	#directVideoMaxBitrate;
	#mediaIdleTimeoutMs;
	#mediaHideGraceMs;
	#shutdownTimeoutMs;
	#encoderFactory;
	#peerCount = 0;
	#mediaCapacityOwners = /* @__PURE__ */ new Map();
	...
	#diagnostics = {
		...
		mediaAttempts: 0,
		mediaFailures: 0,
		currentViewportRevision: void 0,
		currentMediaGeneration: void 0,
	...
	constructor(opts) {
		...
		this.#handshakeTimeoutMs = opts.handshakeTimeoutMs ?? 5e3;
		const profileConfig = {
		...
		this.#stunUrls = validateBrowserStunUrls(opts.stunUrls ?? [...MANAGED_BROWSER_DEFAULT_STUN_URLS]);
		this.#webrtcNegotiationTimeoutMs = positiveStreamInteger(opts.webrtcNegotiationTimeoutMs, 1e4, "webrtcNegotiationTimeoutMs");
		this.#webrtcRetryCooldownMs = nonNegativeStreamInteger(opts.webrtcRetryCooldownMs, 3e4, "webrtcRetryCooldownMs");
		...
		this.#directVideoFrameRate = boundedStreamInteger(opts.directVideoFrameRate, 20, 1, 60, "directVideoFrameRate");
		this.#directVideoMaxBitrate = boundedStreamInteger(opts.directVideoMaxBitrate, MANAGED_BROWSER_DIRECT_VIDEO_MAX_BITRATE, 1, 1e8, "directVideoMaxBitrate");
	...
	handleUpgrade(req, socket, head) {
		...
		this.#server.handleUpgrade(req, socket, head, (ws) => {
			...
			this.#attach(ws, tab, target, profile);
		});
	}
	dispose() {
		this.#disposePromise ??= this.#dispose();
	...
	closeTab(tab) {
		this.#tabConnections.get(this.#runtime.keyOf(tab))?.socket.close(1e3, "Browser Tab closed");
	}
	/** Close the control connection only when it still owns the invalidated target. */
	invalidateTarget(tab, targetIdentity) {
	...
	closeSession(sessionId) {
		for (const [key, connection] of this.#tabConnections) if (key.startsWith(sessionId + ":")) connection.socket.close(1e3, "Browser session disposed");
	}
	resources() {
		return {
	...
	diagnostics() {
		return {
			...
			lastMediaRoute: this.#diagnostics.lastMediaRoute === void 0 ? void 0 : { ...this.#diagnostics.lastMediaRoute },
	...
	async #dispose() {
		...
		for (const socket of sockets) {
			...
			if (cleanup !== void 0) this.#track(cleanup());
		}
		...
		this.#loopbackStun = void 0;
		if (stun !== void 0) await stun.close();
	}
	async #iceStunUrls() {
		...
		if (loopback !== void 0 && !urls.includes(loopback)) urls.unshift(loopback);
		return urls;
	}
	async #drainTasks() {
	...
	#authorize(req) {
		...
		if (!browserStreamRequestAllowed(typeof req.headers.origin === "string" ? req.headers.origin : void 0, host)) return void 0;
		let ticket = null;
		try {
	...
	async #attach(socket, tab, target, profile) {
		...
		if (previous !== void 0 && previous.socket.readyState !== WebSocket.CLOSED) previous.socket.close(4001, "Replaced by a newer stream");
		...
		let pendingMediaLayout;
		let sourceState = "stopped";
		let nextInputSequence = 0;
		let passiveInputBarrier;
		...
		const currentLayout = () => {
			if (currentTarget() === void 0) return void 0;
			return this.#runtime.target(tab, target.identity)?.layout;
		};
		const currentReadTarget = () => {
			if (currentTarget() === void 0) return void 0;
		...
		const sendProjection = () => {
			...
			if (currentTarget() === void 0) return false;
			const projection = this.#runtime.projection(tab);
			if (projection === void 0) return false;
		...
		const sendFrame = (capture, layout) => {
			...
			const payload = profile.frameEncoding === "binary-v2" ? encodeBrowserStreamFrameV2(frame) : encodeBrowserStreamJsonFrameV2(frame);
		...
		const armFrameTimer = (delay, pump) => {
			if (frameTimer !== void 0) return;
			this.#timerCount += 1;
		...
		const pump = () => {
			...
			if (passiveInputBarrier !== void 0 || captureInFlight || unacked !== void 0 || dirty === void 0) return;
			const activeFrameIntervalMs = mediaAttempt?.connected === true ? Math.ceil(1e3 / this.#directVideoFrameRate) : profile.frameIntervalMs;
			...
			const currentCaptureLayout = () => {
				const current = currentReadTarget();
				return current?.layoutEpoch === capturedLayoutEpoch && current.documentId === capturedDocumentId ? current.layout : void 0;
			};
			...
			const task = captureBrowserJpegForLayout(cdp, capturedLayout, currentCaptureLayout, captureProfile, {
				...
				onStaleDrop: () => {
					this.#diagnostics.staleCaptureDrops += 1;
				}
				...
				if (capture === void 0) {
					...
					if (captureRoute === "webrtc-direct") {
						...
						if (attempt?.connected === true && sameMediaLayout(attempt.layout, capturedLayout)) this.#track(failMediaAttempt(attempt, "direct-frame-budget-exceeded"));
						return;
					}
					...
					if (attempt?.connected === true && sameMediaLayout(attempt.layout, capturedLayout)) {
						...
						if (attempt.encoder.submit({
							...
							while (attempt.submittedAt.size > 2) {
								const oldest = attempt.submittedAt.keys().next().value;
				...
				if (sendFrame(capture, capturedLayout)) this.#diagnostics.fallbackBytes += capture.jpeg.byteLength;
				else dirty = { kind: "activity" };
			}).finally(() => {
				if (captureTask === task) captureTask = void 0;
				captureInFlight = false;
				releaseCapture();
				pump();
			});
		...
		const awaitLifecycleCommand = async (task, timeoutMessage) => {
			let timer;
			this.#timerCount += 1;
			const deadline = new Promise((_resolve, reject) => {
				timer = setTimeout(() => {
		...
		const failLifecycle = (error, fallback) => {
			lifecycleTerminal = true;
			mediaVerified = false;
			if (!detached && socket.readyState === WebSocket.OPEN) socket.close(1011, error instanceof Error ? error.message.slice(0, 120) : fallback);
		};
		const clearMediaTimer = (attempt, field) => {
			const timer = attempt[field];
			if (timer === void 0) return;
			clearTimeout(timer);
			attempt[field] = void 0;
			this.#timerCount -= 1;
		};
		const releaseMediaAttempt = async (attempt) => {
			...
			await attempt.encoder.dispose().catch(() => void 0);
		};
		const sendMediaRoute = (route, status, reason) => {
			this.#diagnostics.lastMediaRoute = {
				route,
				status,
				...reason === void 0 ? {} : { reason }
			};
			if (reason !== void 0) this.#diagnostics.mediaRouteReasons[reason] = (this.#diagnostics.mediaRouteReasons[reason] ?? 0) + 1;
			if (route === "jpeg-fallback" && reason !== void 0) console.warn("[dsh-codex-sidebar] Browser media jpeg-fallback", reason);
		...
		const failMediaAttempt = async (attempt, reason) => {
		...
		const handleMediaSignal = (attempt, message) => {
			...
			if (signal.type === "candidate") {
				if (attempt.outboundCandidateCount >= 64) return;
			...
			if (signal.type === "connection-state") {
				if (signal.state === "connected") {
					...
					requestFrame("activity");
				} else if (signal.state === "disconnected" || signal.state === "failed" || signal.state === "closed") this.#track(failMediaAttempt(attempt, "peer-" + signal.state));
				return;
			}
		...
		const startMediaAttempt = async (layout) => {
			...
			if (await this.#reserveMediaCapacity((order) => {
				let owner;
				const encoder = this.#encoderFactory({
					identity: {
						ownerId,
						generation: layout.mediaGeneration
					},
					...
					onSignal: (message) => {
						if (attempt !== void 0) handleMediaSignal(attempt, message);
					}
				});
				owner = {
					ownerId,
					order,
					evictionPriority: () => attempt === void 0 || attempt.released ? void 0 : surfaceHidden ? 0 : attempt.connected ? void 0 : 1,
					evict: async () => {
						const target = attempt;
						if (target === void 0 || mediaAttempt !== target || target.released || !surfaceHidden && target.connected) return false;
				...
				attempt = {
					...
					inboundCandidateCount: 0,
					outboundCandidateCount: 0,
					candidates: [],
					negotiationTimer: void 0,
					idleTimer: void 0,
					submittedAt: /* @__PURE__ */ new Map(),
					capacityOwner: owner,
					released: false
				};
				mediaAttempt = attempt;
				return owner;
			}) === void 0 || attempt === void 0) {
				lastMediaFailureAt = this.#now();
				this.#diagnostics.mediaFailures += 1;
			...
			if (detached || mediaAttempt !== attempt || !sameMediaLayout(currentLayout(), layout)) {
				if (mediaAttempt === attempt) mediaAttempt = void 0;
				await releaseMediaAttempt(attempt);
				return;
			}
			...
			try {
				...
				if (activeAttempt.connected || activeAttempt.negotiationTimer !== void 0) return;
				this.#timerCount += 1;
				activeAttempt.negotiationTimer = setTimeout(() => {
					activeAttempt.negotiationTimer = void 0;
					this.#timerCount -= 1;
		...
		const replaceMediaAttempt = (layout) => {
			const task = mediaTransition.then(async () => {
				const previousAttempt = mediaAttempt;
				mediaAttempt = void 0;
				if (previousAttempt !== void 0) await releaseMediaAttempt(previousAttempt);
				await startMediaAttempt(layout);
			});
			mediaTransition = task.catch(() => void 0);
			this.#track(task);
		};
		noteMediaActivity = () => {
			const attempt = mediaAttempt;
			...
			if (!mediaIdleSuspended || attempt !== void 0 || this.#now() - lastMediaFailureAt < this.#webrtcRetryCooldownMs) return;
			const layout = currentLayout();
			if (layout === void 0) return;
			mediaIdleSuspended = false;
			replaceMediaAttempt(layout);
		};
		const startCommittedMedia = async (layout) => {
			...
			if (detached || lifecycleTerminal || socket.readyState !== WebSocket.OPEN) return;
			if (!sameMediaLayout(currentLayout(), layout) || !sameMediaLayout(pendingMediaLayout, layout)) return;
			sendLayout(layout);
			mediaVerified = true;
			sourceState = "active";
			replaceMediaAttempt(layout);
		...
		const pauseCommittedMedia = async () => {
			mediaVerified = false;
			sourceState = "gated";
		...
		const commitLayout = async (layout) => {
			this.#diagnostics.layoutCommits += 1;
			if (unacked !== void 0 && (unacked.revision !== layout.revision || unacked.mediaGeneration !== layout.mediaGeneration)) {
				unacked = void 0;
				this.#unackedCount -= 1;
			}
			await startCommittedMedia(layout);
		};
		const queueLayoutControl = async (action) => {
			const task = layoutControlTransition.then(action);
			layoutControlTransition = task.catch(() => void 0);
			await task;
		};
		const proposeCommittedLayout = async (proposalSequence, proposal) => {
			try {
				await queueLayoutControl(async () => {
					...
					if (current === void 0 || current.revision !== layout.revision || current.mediaGeneration !== layout.mediaGeneration) return;
		...
		const onFrame = (value) => {
			...
			if (sourceState !== "active" || !mediaVerified) return;
		...
		const detach = async () => {
			...
			dirty = void 0;
			if (unacked !== void 0) this.#unackedCount -= 1;
			unacked = void 0;
			captureInFlight = false;
			releaseCapture();
			const attempt = mediaAttempt;
			...
			releaseLease?.();
			releaseLease = void 0;
			const releaseMedia = attempt === void 0 ? Promise.resolve() : releaseMediaAttempt(attempt);
			const stopScreencast = layoutControlTransition.then(async () => {
				if (!screencastStarted) return;
				screencastStarted = false;
				await awaitLifecycleCommand(cdp.send("Page.stopScreencast"), "Browser screencast cleanup timed out").catch(() => void 0);
			});
			await Promise.all([
				previousCleanup,
				mediaTransition,
				layoutControlTransition,
				releaseMedia,
				stopScreencast,
				captureTask,
				startTask
		...
		socket.on("message", (data) => {
			...
			try {
				...
				if (typeof parsed.type === "string" && parsed.type !== "frame-ack" && parsed.type !== "input" && parsed.type !== "hello") console.warn("[dsh-codex-sidebar] ws-in", parsed.type, message === void 0 ? "decode-fail" : "ok");
			} catch {}
			if (!activated) {
				if (message !== void 0 && message.type !== "hello") socket.close(1008, "Previous Browser owner is still detaching");
				return;
			}
			if (message?.type === "frame-ack") {
				if (unacked !== void 0 && message.sequence === unacked.sequence && message.revision === unacked.revision && message.mediaGeneration === unacked.mediaGeneration) {
					recordLatency(this.#diagnostics.fallbackAckEndToEndLatencyMs, this.#now() - unacked.sentAt);
					unacked = void 0;
					this.#unackedCount -= 1;
					if (dirty?.kind === "passive") armFrameTimer(1, pump);
					else pump();
				}
				return;
			}
			if (message === void 0 || message.type === "hello") return;
			if (message.type === "surface-visibility") {
				const layout = currentLayout();
				if (mediaStarted && layout !== void 0 && message.ownerId === ownerId && message.revision === layout.revision && message.mediaGeneration === layout.mediaGeneration) surfaceHidden = !message.visible;
				return;
			}
			if (message.type === "rtc-answer" || message.type === "rtc-candidate" || message.type === "media-retry" || message.type === "media-decline") {
				if (message.type === "media-retry") {
					const layout = currentLayout();
					const now = this.#now();
					if (!mediaStarted || !mediaVerified || layout === void 0 || message.ownerId !== ownerId || message.revision !== layout.revision || message.mediaGeneration !== layout.mediaGeneration || mediaAttempt?.connected === true && message.trigger !== "explicit" || now - Math.max(lastMediaFailureAt, lastMediaRetryAt) < this.#webrtcRetryCooldownMs) return;
				...
				if (message.type === "media-decline") {
					const attempt = mediaAttempt;
					if (attempt === void 0 || message.ownerId !== ownerId || message.revision !== attempt.layout.revision || message.mediaGeneration !== attempt.layout.mediaGeneration) return;
					if (message.detail !== void 0) console.warn("[dsh-codex-sidebar] client media-decline detail", message.reason, message.detail);
					this.#track(failMediaAttempt(attempt, "client-" + message.reason));
					return;
				}
				const attempt = mediaAttempt;
				if (attempt === void 0 || message.ownerId !== ownerId || message.revision !== attempt.layout.revision || message.mediaGeneration !== attempt.layout.mediaGeneration) {
					console.warn("[dsh-codex-sidebar] rtc drop", message.type, {
						haveAttempt: attempt !== void 0,
				...
				if (message.type === "rtc-candidate") {
					if (attempt.inboundCandidateCount >= 64) return;
			...
			const inputSequence = message.type === "input" ? ++nextInputSequence : void 0;
			if (inputSequence !== void 0 && dirty?.kind === "passive" && !captureInFlight && passiveInputBarrier === void 0) passiveInputBarrier = inputSequence;
			const messageTask = this.#onMessage(socket, tab, target.identity, message, requestFrame, proposeCommittedLayout, currentTarget, currentLayout, noteMediaActivity, {
				get latest() {
					return latestProposalSequence;
				},
				set latest(value) {
					latestProposalSequence = value;
				}
			}).catch(() => {
				currentTarget();
			}).finally(() => {
				if (inputSequence === void 0 || passiveInputBarrier !== inputSequence) return;
				passiveInputBarrier = void 0;
				pump();
			});
		...
		const activate = async () => {
			...
			if (!handshaken || startTask !== void 0) return;
			startTask = start();
			await startTask;
		};
		this.#socketCleanup.set(socket, connection.cleanup);
		socket.once("close", () => {
			this.#track(connection.cleanup());
		});
		socket.once("error", () => {
			this.#track(connection.cleanup());
		});
	...
	async #onMessage(socket, tab, targetIdentity, message, requestFrame, proposeCommittedLayout, currentTarget, currentLayout, noteMediaActivity, proposal) {
		...
		if (message.type === "outline") {
			...
			if (currentTarget() !== void 0 && "nodes" in outline && socket.readyState === WebSocket.OPEN) socket.send(JSON.stringify({
				type: "outline",
		...
		if (message.type === "layout-propose") {
			...
			await proposeCommittedLayout(message.proposalSequence, {
				mode: message.mode,
		...
		const layout = currentLayout();
		if (layout === void 0 || message.revision !== layout.revision) {
		...
		if (inputTarget === void 0) return;
		const inputLayoutIsCurrent = () => {
			const current = this.#runtime.target(tab, targetIdentity);
			return current?.layoutEpoch === inputTarget.layoutEpoch && current.documentId === inputTarget.documentId && current.layout.revision === message.revision;
		};
		noteMediaActivity();
		this.#runtime.touch(tab);
		if (!await this.#runtime.runInput(tab, targetIdentity, {
		...
		if (message.input.type === "wheel" && message.input.selector !== void 0) {
			...
			if (inputLayoutIsCurrent() && "rect" in tracked && socket.readyState === WebSocket.OPEN) socket.send(JSON.stringify({
				type: "tracked-rect",
				...tracked
			}));
		}
	...
	async #reserveMediaCapacity(create) {
		const reservation = this.#mediaCapacityTransition.then(async () => {
			while (this.#peerCount >= this.#maxMediaPeers) {
				const candidates = [...this.#mediaCapacityOwners.values()].map((owner) => ({
					owner,
					priority: owner.evictionPriority()
				})).filter((candidate) => candidate.priority !== void 0).sort((left, right) => left.priority - right.priority || left.owner.order - right.owner.order);
				let evicted = false;
				for (const candidate of candidates) if (await candidate.owner.evict()) {
					evicted = true;
					break;
				}
				if (!evicted) return void 0;
			}
		...
		this.#mediaCapacityTransition = reservation.then(() => void 0, () => void 0);
		return await reservation;
	}
	#releaseMediaCapacity(owner) {
		if (this.#mediaCapacityOwners.get(owner.ownerId) !== owner) return;
		this.#mediaCapacityOwners.delete(owner.ownerId);
		this.#peerCount -= 1;
	}
};
function browserStreamVisualViewportOrigin(value) {
	if (typeof value !== "object" || value === null) return {
		x: 0,
		y: 0
	};
	const viewport = value.visualViewport;
	if (typeof viewport !== "object" || viewport === null) return {
		x: 0,
		y: 0
	};
...
async function captureBrowserJpegForLayout(cdp, layout, currentLayout, profile, observer = {}, deviceScaleFactor = 1) {
	const isCurrent = () => {
...
async function captureBrowserJpegWithinBudget(cdp, viewport, profile, observer = {}, deviceScaleFactor = 1) {
	if (observer.isCurrent?.() === false) return void 0;
	const metrics = await cdp.send("Page.getLayoutMetrics").catch(() => void 0);
	if (observer.isCurrent?.() === false) return void 0;
	...
	const attempts = uniqueCaptureAttempts([
		...
		{
			quality: Math.min(profile.quality, 60),
			scale: Math.min(preferredScale, 1)
		},
		{
			quality: Math.min(profile.quality, 45),
			scale: Math.min(preferredScale, .75)
		},
		{
			quality: Math.min(profile.quality, 30),
			scale: Math.min(preferredScale, .5)
		}
	]);
...
function uniqueCaptureAttempts(attempts) {
	const seen = /* @__PURE__ */ new Set();
...
async function waitForBrowserPaint(cdp) {
...
*/
async function dispatchBrowserInput(cdp, input, targetIsCurrent = () => true) {
	if (input.type === "wheel") {
...
function browserStreamRequestAllowed(origin, host) {
	if (host === void 0 || host.length === 0) return false;
	if (origin === void 0 || origin.length === 0) return true;
	try {
...
function sameMediaLayout(left, right) {
	return left !== void 0 && right !== void 0 && left.revision === right.revision && left.mediaGeneration === right.mediaGeneration;
}
function positiveStreamInteger(value, fallback, name) {
	if (value === void 0) return fallback;
	if (!Number.isSafeInteger(value) || value <= 0) throw new Error("managedBrowser " + name + " must be a positive safe integer");
	return value;
}
function nonNegativeStreamInteger(value, fallback, name) {
	if (value === void 0) return fallback;
	if (!Number.isSafeInteger(value) || value < 0) throw new Error("managedBrowser " + name + " must be a non-negative safe integer");
	return value;
}
function emptyLatencyDiagnostic() {
	return {
		samples: 0,
		totalMs: 0,
		lastMs: 0,
...
function recordLatency(metric, elapsedMs) {
...
function boundedStreamInteger(value, fallback, min, max, name) {
	const resolved = value ?? fallback;
	if (!Number.isSafeInteger(resolved) || resolved < min || resolved > max) throw new Error("managedBrowser " + name + " must be an integer from " + min + " to " + max);
	return resolved;
}
function jpegQuality(value, fallback, name) {
	return boundedStreamInteger(value, fallback, 1, 100, name);
}
function jpegScale(value, fallback, name) {
	const resolved = value ?? fallback;
	if (!Number.isFinite(resolved) || resolved <= 0 || resolved > 4) throw new Error("managedBrowser " + name + " must be a finite number greater than 0 and at most 4");
	return resolved;
}
function browserStreamCaptureScale(width, height, maxScale = HIGH_DENSITY_SCALE) {
	if (!Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) return 1;
	return Math.max(.1, Math.min(maxScale, MANAGED_BROWSER_STREAM_MAX_WIDTH / width, MANAGED_BROWSER_STREAM_MAX_HEIGHT / height));
}
...
function rawByteBudget(value, fallback, name = "JPEG raw-byte budget") {
	if (value === void 0) return fallback;
	if (!Number.isSafeInteger(value) || value <= 0) throw new Error("managedBrowser " + name + " must be a positive safe integer");
	return value;
}
...
function createManagedBrowserDriveService(runtime) {
	...
	const listed = (session) => {
		...
		return snapshot.tabs.flatMap((tab) => {
			...
			return [{
				tabId: tab.id,
				url: managedBrowserHref(url) ?? url,
	...
	return {
		...
		async open(caller, session, url) {
			...
			if (href === void 0) return {
				ok: false,
				code: "navigation-failed",
				message: "需要 http、https 或绝对本地 HTML 地址"
			};
...
function stripAnnotationDraftSentinel(draft) {
	if (!draft.endsWith("​")) return draft;
	const withoutSentinel = draft.slice(0, -1);
...
function enrichUserMessage(message, batch) {
	...
	return {
		...
		content: [...message.content.map((block) => block.type === "text" ? {
			...block,
			text: stripAnnotationDraftSentinel(block.text)
		} : block), ...evidence],
		source
	};
}
...
function decodeEvidence$1(value) {
	if (typeof value !== "object" || value === null || Array.isArray(value)) return void 0;
	const rec = value;
	if (typeof rec.id !== "string" || typeof rec.captureId !== "string" || typeof rec.documentId !== "string" || !Number.isSafeInteger(rec.layoutRevision) || rec.layoutRevision <= 0 || !Number.isSafeInteger(rec.mediaGeneration) || rec.mediaGeneration <= 0 || typeof rec.ref !== "string" || rec.mediaType !== "image/jpeg" || typeof rec.width !== "number" || typeof rec.height !== "number") return void 0;
	return {
		id: rec.id,
		captureId: rec.captureId,
		documentId: rec.documentId,
		layoutRevision: rec.layoutRevision,
		mediaGeneration: rec.mediaGeneration,
		ref: rec.ref,
		mediaType: "image/jpeg",
		width: rec.width,
		height: rec.height
	...
	return {
		...
		async project(snapshot, gate, signal) {
			...
			if (active?.kind === "Files") {
				...
				const previewPromise = selectedPath.length > 0 && !selectedImage ? readPreview(gate.cwd, selectedPath, signal) : Promise.resolve(void 0);
				...
				const preview = path === selectedPath ? await previewPromise : image ? void 0 : await readPreview(gate.cwd, path, signal);
...
async function readPreview(cwd, path, signal) {
	...
	if (expanded.length === 0 || cwd.length === 0 && !isAbsolute(expanded)) return void 0;
...
async function handleSidebarRpcAsync(registry, endpoint, payload, services = {}) {
	try {
		...
		if (endpoint === "sidebar/browser-stream-ticket" && services.managedBrowser !== void 0) {
			...
			if (projection.status === "error" || projection.status === "crashed") return fail(projection.error ?? "Browser page is not ready");
			return {
				ok: true,
		...
		if (endpoint === "sidebar/browser-capture") {
			if (!isRecord(payload) || typeof payload.sessionId !== "string" || typeof payload.tabId !== "string" || !positiveSafeInteger(payload.expectedRevision) || !positiveSafeInteger(payload.expectedMediaGeneration)) return fail("invalid sidebar browser-capture request");
			...
			if (registry.forSession(payload.sessionId, {
				cwd: typeof payload.cwd === "string" ? payload.cwd : "",
				busy: payload.busy === true
			}).snapshot(false).tabs.find((item) => item.id === tabId && item.kind === "Browser") === void 0) return fail("unknown Browser Tab");
			return {
				ok: true,
				value: await services.browserEvidence.capture({
					...
					mediaGeneration: payload.expectedMediaGeneration
				})
			};
		}
		if (endpoint === "sidebar/browser-evidence-commit") {
			if (!isRecord(payload) || typeof payload.sessionId !== "string" || typeof payload.captureId !== "string" || !positiveSafeInteger(payload.expectedRevision) || !positiveSafeInteger(payload.expectedMediaGeneration)) return fail("invalid sidebar browser-evidence-commit request");
		...
		if (endpoint === "sidebar/browser-evidence-read") {
			if (!isRecord(payload) || typeof payload.sessionId !== "string" || typeof payload.offset !== "number" || !Number.isSafeInteger(payload.offset) || payload.offset < 0) return fail("invalid sidebar browser-evidence-read request");
			...
			return {
				ok: true,
				value: await services.browserEvidence.readChunk(payload.sessionId, evidence, payload.offset)
			};
		}
...
function decodeEvidence(value) {
	...
	if (typeof value.id !== "string" || typeof value.captureId !== "string" || typeof value.documentId !== "string" || !positiveSafeInteger(value.layoutRevision) || !positiveSafeInteger(value.mediaGeneration) || typeof value.ref !== "string" || value.mediaType !== "image/jpeg" || typeof value.width !== "number" || typeof value.height !== "number") return void 0;
	return {
		id: value.id,
...
function withHostSessionCwd(payload, cwdForSession) {
	if (!isRecord(payload) || typeof payload.sessionId !== "string" || cwdForSession === void 0) return payload;
	const cwd = cwdForSession(payload.sessionId);
	return cwd === void 0 || cwd.length === 0 ? payload : {
		...payload,
		cwd
	};
}
...
function createSidebarSession(opts) {
	...
	function applyBrowser(intent) {
		const requestedTabId = "tabId" in intent && typeof intent.tabId === "string" ? intent.tabId : void 0;
		const id = requestedTabId ?? browserTabId();
		if (requestedTabId !== void 0 && !tabs.some((tab) => tab.id === requestedTabId && tab.kind === "Browser")) return void 0;
	...
	function dispatch(intent) {
		...
		switch (intent.type) {
			...
			case "browser-note-add": {
				const requestedTabId = intent.tabId;
				const id = requestedTabId ?? browserTabId();
				if (requestedTabId !== void 0 && !tabs.some((tab) => tab.id === requestedTabId && tab.kind === "Browser")) break;
...
function apply(ctx, config = {}) {
	...
	const managedStream = new ManagedBrowserStream({
		...
		...config.managedBrowser?.desktopJpegMaxRawBytes === void 0 ? {} : { desktopMaxRawBytes: config.managedBrowser.desktopJpegMaxRawBytes },
		...config.managedBrowser?.mobileJpegMaxRawBytes === void 0 ? {} : { mobileMaxRawBytes: config.managedBrowser.mobileJpegMaxRawBytes },
		...config.managedBrowser?.desktopJpegQuality === void 0 ? {} : { desktopJpegQuality: config.managedBrowser.desktopJpegQuality },
		...config.managedBrowser?.desktopJpegFrameIntervalMs === void 0 ? {} : { desktopJpegFrameIntervalMs: config.managedBrowser.desktopJpegFrameIntervalMs },
		...config.managedBrowser?.desktopJpegMaxScale === void 0 ? {} : { desktopJpegMaxScale: config.managedBrowser.desktopJpegMaxScale },
		...config.managedBrowser?.desktopScreencastEveryNthFrame === void 0 ? {} : { desktopScreencastEveryNthFrame: config.managedBrowser.desktopScreencastEveryNthFrame },
		...config.managedBrowser?.desktopJpegInteractionBurstFrames === void 0 ? {} : { desktopJpegInteractionBurstFrames: config.managedBrowser.desktopJpegInteractionBurstFrames },
		...config.managedBrowser?.mobileJpegQuality === void 0 ? {} : { mobileJpegQuality: config.managedBrowser.mobileJpegQuality },
		...config.managedBrowser?.mobileJpegFrameIntervalMs === void 0 ? {} : { mobileJpegFrameIntervalMs: config.managedBrowser.mobileJpegFrameIntervalMs },
		...config.managedBrowser?.mobileJpegMaxScale === void 0 ? {} : { mobileJpegMaxScale: config.managedBrowser.mobileJpegMaxScale },
		...config.managedBrowser?.mobileScreencastEveryNthFrame === void 0 ? {} : { mobileScreencastEveryNthFrame: config.managedBrowser.mobileScreencastEveryNthFrame },
		...config.managedBrowser?.mobileJpegInteractionBurstFrames === void 0 ? {} : { mobileJpegInteractionBurstFrames: config.managedBrowser.mobileJpegInteractionBurstFrames },
		...config.managedBrowser?.preferredMediaRoute === void 0 ? {} : { preferredMediaRoute: config.managedBrowser.preferredMediaRoute },
		...config.managedBrowser?.stunUrls === void 0 ? {} : { stunUrls: config.managedBrowser.stunUrls },
		...config.managedBrowser?.webrtcNegotiationTimeoutMs === void 0 ? {} : { webrtcNegotiationTimeoutMs: config.managedBrowser.webrtcNegotiationTimeoutMs },
		...config.managedBrowser?.webrtcRetryCooldownMs === void 0 ? {} : { webrtcRetryCooldownMs: config.managedBrowser.webrtcRetryCooldownMs },
		...config.managedBrowser?.maxMediaPeers === void 0 ? {} : { maxMediaPeers: config.managedBrowser.maxMediaPeers },
		...config.managedBrowser?.maxEncoderPages === void 0 ? {} : { maxEncoderPages: config.managedBrowser.maxEncoderPages },
		...config.managedBrowser?.directVideoFrameRate === void 0 ? {} : { directVideoFrameRate: config.managedBrowser.directVideoFrameRate },
		...config.managedBrowser?.directVideoMaxBitrate === void 0 ? {} : { directVideoMaxBitrate: config.managedBrowser.directVideoMaxBitrate },
		...config.managedBrowser?.directVideoCaptureQuality === void 0 ? {} : { directVideoCaptureQuality: config.managedBrowser.directVideoCaptureQuality },
		...config.managedBrowser?.directVideoCaptureMaxScale === void 0 ? {} : { directVideoCaptureMaxScale: config.managedBrowser.directVideoCaptureMaxScale },
		...config.managedBrowser?.directVideoCaptureMaxRawBytes === void 0 ? {} : { directVideoCaptureMaxRawBytes: config.managedBrowser.directVideoCaptureMaxRawBytes },
		...config.managedBrowser?.mediaIdleTimeoutMs === void 0 ? {} : { mediaIdleTimeoutMs: config.managedBrowser.mediaIdleTimeoutMs },
		...config.managedBrowser?.mediaHideGraceMs === void 0 ? {} : { mediaHideGraceMs: config.managedBrowser.mediaHideGraceMs },
		...config.managedBrowser?.browserCleanupTimeoutMs === void 0 ? {} : { shutdownTimeoutMs: config.managedBrowser.browserCleanupTimeoutMs }
	});
	...
	ctx.effect(() => installManagedBrowserSessionLifecycle(ctx, managedStream, managedBrowser, filesBySession), "dsh-codex-sidebar: managed browser session lifecycle");
