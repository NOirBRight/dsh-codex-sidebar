...
export class ManagedBrowserEvidenceStore {
  ...
  async capture(tab: ManagedTabKey): Promise<BrowserCaptureMetadata> {
    this.#pruneTemporary()
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      if (projection?.documentId !== result.documentId) continue
  ...
  async commit(sessionId: string, captureId: string): Promise<BrowserEvidence> {
    this.#pruneTemporary()
