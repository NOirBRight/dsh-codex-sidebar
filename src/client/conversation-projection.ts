...
export function createConversationProjection(
  ...
  let version = 0
  let parserVersion = -1
  let parserSnapshot: ParserSnapshot | undefined
  let rowVersion = -1
  ...
  const parserFor = (state: ProjectionState): ParserSnapshot | undefined => {
  ...
  const rowHunks = (): readonly RowHunkStat[] => {
    ...
    if (rowVersion !== state.version) {
      ...
      rows = parser === undefined ? [] : rowHunksFromSnapshot(parser)
    }
    return rows
  }
  ...
  const turnWrites = (): readonly ReviewChange[] => {
    ...
    if (writeVersion !== state.version) {
      ...
      const parser = parserFor(state)
      writes = parser === undefined ? [] : turnWritesFromSession(parser)
    }
    return writes
  }
  ...
  const logEvents = (): readonly LogEvent[] => {
    ...
    if (logSource !== source) {
      ...
      logs = parser === undefined ? [] : logEventsFromSession(parser)
    }
    return logs
  }
  ...
  return {
    ...
    hunkForOpen: (path, tool, hunkId) => {
      ...
      return parser === undefined ? undefined : findHunkForOpen(parser, path, tool, hunkId)
    },
  }
}
