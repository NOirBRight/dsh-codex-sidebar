...
export function decodeBrowserClientMessage(raw: string): BrowserClientMessage | undefined {
  ...
  if (value.type === 'media-decline') {
    if (identity === undefined || (value.reason !== 'presentation-failed' && value.reason !== 'negotiation-timeout'
      && value.reason !== 'negotiation-error' && value.reason !== 'ready-missing'
  ...
  if (value.type === 'surface-visibility') {
    ...
    return { type: 'surface-visibility', ...identity, visible: value.visible }
  }
