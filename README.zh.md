...
```sh
dsh plugin --profile web add github:NOirBRight/dsh-codex-sidebar#v0.5.5
dsh web
```
...
```sh
DSH_HOME=~/.dsh-lab dsh plugin --profile web add github:NOirBRight/dsh-codex-sidebar#v0.5.5
```

仓库里带发布用的 `lib/` 产物，从 GitHub 安装不必放行构建脚本。

0.5.5 增加 127.0.0.1 loopback STUN：编码器 Chromium 走 fake-ip/TUN（198.18.0.0/15）而 GUI Chrome 不在同一网时，Direct video 仍能建连。
...
phone、tablet、laptop 三个固定预设仍为 `390×844`、`768×1024`、`1280×800`。fit 模式只在容器稳定后提交一次受限 viewport；选择固定预设只通过其 v2 控制连接发送一次 proposal，持久化会话和 ticket 路径不会直接提交布局。固定预设不读取容器 resize。Host 会在每次 proposal 后验证精确目标 Page 的 CSS viewport，未变化的 proposal 也不例外；固定布局连接首次启动 screencast 后还会再次验证。重连验证完成前，该 screencast 不会触发截图，也不会启动直连媒体协商。viewport 过渡期间会暂停视觉读取、媒体截图和输入，但不会关闭控制连接；内部 epoch 会丢弃跨越未变化布局验证的旧工作，视觉结果和手势还会复核其来源 document。客户端把桌面 press/release 作为一个原子 tap 或 drag 发送，Host 再把完整手势与 viewport 写入串行；批注框选的 press 和 release 如果使用了不同的 presented revision，会取消本次框选。已完成的操作不会仅因后续过渡已排队就误报失败。Mobile 完成一次 touch tap 后才打开隐藏 IME，并暂停 fit proposal 直至 blur；touch 滚动和桌面 focus 不会暂停 fit。重连检查会修复实际 viewport 漂移，同时保留原 revision 和 media generation。如果 Playwright 已完成但尺寸未生效，Host 会通过绑定同一 identity 的 CDP metrics override 重试，仍无法验证时关闭目标而不是发布或传输虚假布局。每条控制连接只绑定一个精确的 managed Page/CDP identity；目标被替换时连接会断开。WebRTC 只传视频，不请求摄像头、麦克风或音频。`stunUrls` 只接受 `stun:` URL，拒绝 TURN；空列表仍可使用 Host ICE candidate，需要 NAT discovery 的部署必须配置获准的 STUN 服务。诊断时可把 `preferredMediaRoute` 设为 `jpeg-only`。

无 Origin 的 Mobile 隧道会在 Browser JSON frame 外再套一层 Base64。默认 96 KiB 上限针对编码后的 JPEG 字节，完整 tunnel plaintext 仍低于 200 KiB 限制。Browser evidence 仍是独立、精确的 Page capture；其读取 RPC 必须带明确 offset，每次最多返回 96 KiB 原始 JPEG 的 Base64 和下一 offset，不会通过隧道发送数 MiB 的单次响应。Fallback 可以降低 JPEG quality 或编码分辨率，但绝不改变已提交的 CSS viewport。`desktopJpegFrameIntervalMs` 和 `mobileJpegFrameIntervalMs` 是捕获速率硬上限，交互触发的 frame 也不能绕过。每次交互、导航、刷新或布局提交后，最多再允许 `desktopJpegInteractionBurstFrames` 或 `mobileJpegInteractionBurstFrames` 个被动 screencast 更新；预算耗尽后，纯动画页面停止传帧。新活动会补满预算并保留最新 dirty update。WebRTC 直连视频使用独立的 `directVideoCaptureQuality`、`directVideoCaptureMaxScale` 和 `directVideoCaptureMaxRawBytes` profile，从 Host 已提交 viewport 捕获，不受控制 socket 是否带 Origin 影响；这些仅供 encoder 使用的 JPEG 字节不会进入 Mobile 隧道。每条连接最多保留一个 capture、一个未确认 frame 和一个 latest dirty request。
...
以上数值均为默认值。`mediaIdleTimeoutMs` 会释放无活动的直连视频 peer，但保留目标 Page；后续交互可在重试冷却期结束后重新协商。当文档或 Browser surface 变为隐藏时，`mediaHideGraceMs` 会在短暂恢复窗口内保留控制连接。切换到其他工具 Tab 时，Browser surface 在这段时间内保持挂载，但处于 hidden、inert 状态，不占布局也不接收输入。到期前恢复可取消回收；到期后会关闭控制连接并释放对应 peer 和 encoder，但不会关闭目标 Page。`browserCleanupTimeoutMs` 会限制普通 Tab 关闭、Page 创建失败和插件关闭等待 Browser 自有清理的时长。插件关闭会先立即撤销本地 HTML capability，再等待 stream socket、Chromium target 和其他自有任务。
