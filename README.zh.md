...
phone、tablet、laptop 三个固定预设仍为 `390×844`、`768×1024`、`1280×800`。fit 模式只在容器稳定后提交一次受限 viewport；选择固定预设只通过其 v2 控制连接发送一次 proposal，持久化会话和 ticket 路径不会直接提交布局。固定预设不读取容器 resize。Host 会在每次 proposal 后验证精确目标 Page 的 CSS viewport，未变化的 proposal 也不例外；固定布局连接首次启动 screencast 后还会再次验证。重连验证完成前，该 screencast 不会触发截图，也不会启动直连媒体协商。viewport 过渡期间会暂停视觉读取、媒体截图和输入，但不会关闭控制连接；内部 epoch 会丢弃跨越未变化布局验证的旧工作，视觉结果和手势还会复核其来源 document。客户端把桌面 press/release 作为一个原子 tap 或 drag 发送，Host 再把完整手势与 viewport 写入串行；已完成的操作不会仅因后续过渡已排队就误报失败。Mobile 完成一次 touch tap 后才打开隐藏 IME，并暂停 fit proposal 直至 blur；touch 滚动和桌面 focus 不会暂停 fit。重连检查会修复实际 viewport 漂移，同时保留原 revision 和 media generation。如果 Playwright 已完成但尺寸未生效，Host 会通过绑定同一 identity 的 CDP metrics override 重试，仍无法验证时关闭目标而不是发布或传输虚假布局。每条控制连接只绑定一个精确的 managed Page/CDP identity；目标被替换时连接会断开。WebRTC 只传视频，不请求摄像头、麦克风或音频。`stunUrls` 只接受 `stun:` URL，拒绝 TURN；空列表仍可使用 Host ICE candidate，需要 NAT discovery 的部署必须配置获准的 STUN 服务。诊断时可把 `preferredMediaRoute` 设为 `jpeg-only`。
